...
// ================== Konfigurasi WiFi AP ==================
const char* AP_SSID = "Antariksa";     // Nama WiFi
const char* AP_PASS = "123456789";     // Password min 8 karakter
...
// ================== Status Robot ==================
bool robotAktif = false;  // Status ON/OFF robot
...
void handleRoot() {
  ...
  html += "<h1>Kontrol Robot Sumo</h1>";
  html += "<h2>Status: " + String(robotAktif ? "ON" : "OFF") + "</h2>";
  html += "<div>";
  html += "<a class='btn on' href='/on'>ON</a>";
  html += "<a class='btn stop' href='/off'>OFF</a>";
  html += "</div>";
...
void setup() {
  ...
  server.on("/on", [](){ robotAktif = true; server.send(200,"text/plain","Robot ON"); });
  server.on("/off", [](){ robotAktif = false; berhenti(); server.send(200,"text/plain","Robot OFF"); });
